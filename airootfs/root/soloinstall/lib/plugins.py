...
# 1: List soloinstall.plugin definitions
# 2: Load the plugin entrypoint
# 3: Initiate the plugin and store it as .name in plugins
for plugin_definition in metadata.entry_points().select(group='soloinstall.plugin'):
...
def _localize_path(path: Path) -> Path:
	...
	if url.scheme and url.scheme in ('https', 'http'):
		...
		hashlib.md5(os.urandom(12), usedforsecurity=False).hexdigest()
...
def load_plugin(path: Path) -> None:
	...
	if namespace and namespace in sys.modules:
		...
		# Version dependency via __soloinstall__version__ variable (if present) in the plugin
		...
		if version is not None:
			...
			if sys.modules[namespace].__soloinstall__version__ < float(version_major_and_minor):
