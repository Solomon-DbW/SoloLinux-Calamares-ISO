...
class Pacman:
	...
	def run(args: str, default_cmd: str = 'pacman') -> SysCommand:
		...
		while pacman_db_lock.exists():
			...
			if time.time() - started > (60 * 10):
				error(tr('Pre-existing pacman lock never exited. Please clean up any existing pacman sessions before using soloinstall.'))
				exit(1)
	...
	def strap(self, packages: str | list[str]) -> None:
		...
		self.ask(
			...
			'Pacstrap failed. See /var/log/soloinstall/install.log or above message for error details',
			SysCommand,
