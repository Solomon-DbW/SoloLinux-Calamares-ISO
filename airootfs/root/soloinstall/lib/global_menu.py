...
from soloinstall.lib.disk.disk_menu import DiskLayoutConfigurationMenu
...
from soloinstall.lib.models.device import DiskLayoutConfiguration, DiskLayoutType, EncryptionType, FilesystemType, PartitionModification
from soloinstall.lib.packages import list_available_packages
...
class GlobalMenu(AbstractMenu[None]):
	...
	def _get_menu_options(self) -> list[MenuItem]:
		menu_options = [
			MenuItem(
				text=tr('SoloInstall language'),
				action=self._select_soloinstall_language,
				display_action=lambda x: x.display_name if x else '',
				key='soloinstall_language',
			),
			MenuItem(
				text=tr('Locales'),
	...
	def _select_soloinstall_language(self, preset: Language) -> Language:
		from .interactions.general_conf import select_soloinstall_language

		language = select_soloinstall_language(translation_handler.translated_languages, preset)
