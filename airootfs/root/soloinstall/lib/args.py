...
from soloinstall.lib.crypt import decrypt
from soloinstall.lib.models.application import ApplicationConfiguration
from soloinstall.lib.models.authentication import AuthenticationConfiguration
from soloinstall.lib.models.bootloader import Bootloader
from soloinstall.lib.models.device import DiskEncryption, DiskLayoutConfiguration
from soloinstall.lib.models.locale import LocaleConfiguration
from soloinstall.lib.models.mirrors import MirrorConfiguration
from soloinstall.lib.models.network import NetworkConfiguration
from soloinstall.lib.models.packages import Repository
from soloinstall.lib.models.profile import ProfileConfiguration
from soloinstall.lib.models.users import Password, User, UserSerialization
from soloinstall.lib.output import debug, error, logger, warn
from soloinstall.lib.plugins import load_plugin
from soloinstall.lib.translationhandler import Language, tr, translation_handler
from soloinstall.lib.utils.util import get_password
...
class ArchConfig:
	...
	soloinstall_language: Language = field(default_factory=lambda: translation_handler.get_language_by_abbr('en'))
	...
	def safe_json(self) -> dict[str, Any]:
		config: Any = {
			...
			'script': self.script,
			'soloinstall-language': self.soloinstall_language.json(),
	...
	def from_config(cls, args_config: dict[str, Any], args: Arguments) -> 'ArchConfig':
		...
		if soloinstall_lang := args_config.get('soloinstall-language', None):
			arch_config.soloinstall_language = translation_handler.get_language_by_name(soloinstall_lang)
...
class ArchConfigHandler:
	...
	def _define_arguments(self) -> ArgumentParser:
		...
		parser.add_argument(
			...
			action='store_true',
			default=False,
			help='Skip the version check when running soloinstall',
		)
		parser.add_argument(
			'--skip-wifi-check',
			action='store_true',
			default=False,
			help='Skip wifi check when running soloinstall',
		)
		parser.add_argument(
			'--advanced',
			action='store_true',
