...
from soloinstall.lib.network.wifi_handler import wifi_handler
from soloinstall.lib.networking import ping
from soloinstall.lib.packages.packages import check_package_upgrade
from soloinstall.tui.ui.components import tui as ttui
...
# @soloinstall.plugin decorator hook to programmatically add
...
def _fetch_arch_db() -> None:
	...
	try:
		...
		error('Run soloinstall --debug and check /var/log/soloinstall/install.log for details.')
...
def check_version_upgrade() -> str | None:
	...
	if upgrade is None:
		debug('No soloinstall upgrades found')
		return None
...
def main() -> int:
	...
	OR straight as a module: python -m soloinstall
	...
	if os.getuid() != 0:
		print(tr('SoloInstall requires root privileges to run. See --help for more.'))
		return 1

	_log_sys_info()

	ttui.global_header = 'SoloInstall'
	...
	mod_name = f'soloinstall.scripts.{script}'
...
def run_as_a_module() -> None:
	...
	try:
		...
		if exc:
			...
			text = (
				'SoloInstall experienced the above error. If you think this is a bug, please report it to\n'
				'https://github.com/archlinux/soloinstall and include the log file "/var/log/soloinstall/install.log".\n\n'
				"Hint: To extract the log from a live ISO \ncurl -F'file=@/var/log/soloinstall/install.log' https://0x0.st\n"
			)
